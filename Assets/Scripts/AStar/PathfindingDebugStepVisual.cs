        ...
        for (int x = 0; x < grid.GetWidth(); x++) {
            for (int y = 0; y < grid.GetHeight(); y++) {
                ...
                gridSnapshotAction.AddAction(() => {
                    ...
                    Color backgroundColor = UtilsMethod.GetColorFromString("636363");

                    if (isInClosedList) {
                        backgroundColor = new Color(1, 0, 0);
                    }
                    if (isInOpenList) {
                        backgroundColor = UtilsMethod.GetColorFromString("009AFF");
                    }
                    if (isCurrent) {
            ...
            for (int y = 0; y < grid.GetHeight(); y++) {
                ...
                gridSnapshotAction.AddAction(() => { 
                    ...
                    if (isInPath) {
                        ...
                        backgroundColor = UtilsMethod.GetColorFromString("636363");
                    }
